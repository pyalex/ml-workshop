...
object Streaming {
  ...
  private val token = "ya29.Glx_BVMKtlb7ULxEZLCXxA8Ysb7qykWyvGeSHDsIKnUJBp0BalxwDc7HHeg" +
    "-ub0EoB78jrJ7avFeu-BOKB5zP8s2qojRLkti42-8eB5TcZ41N0nC2qfmYHEuzqHeZw"
  ...
  def toEmails(ids: Set[MessageKey]): Seq[Email] = {
    ids.flatMap{
      key => gmailClient.getMessage(key.messageId)
        ...
        .map(_.copy(thread = Some(key.threadId)))
    }.toSeq
  }
...
//    )
    ...
    windowed.map(x => (x.thread.getOrElse(""), 1)).sumByKey
  }
  ...
  def withThreadAnswer(emails: SCollection[Email], originator: String): SCollection[String] = {
    ...
    windowed.filter(_.from == originator).flatMap(_.thread).distinct
  }
...
//      case (e, ctx) => (e.thread, e.subject, ctx(lengthByThread).get(e.thread.getOrElse("")))
//    }.toSCollection.debug()
